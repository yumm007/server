...
echo SCAN_RES_2: $SCAN_RES_2 \($TOTAL_2/$TOTAL\)
...
echo TOTAL_ON: $ON \($on/\($on+$off\)\)

ap=`head ap.list -n 1`
AVE=`grep A=$ap $2 | cut -d';' -f3 | cut -b 8-| awk '{n++;sum+=$1};END{printf "%0.0f", sum/n}'`

echo RSSI_AVE: $AVE dBm
